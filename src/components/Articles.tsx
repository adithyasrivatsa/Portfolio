...
export default function Articles() {
  ...
  return (
    <section id="articles" className="py-16 md:py-24 border-b-8 border-black relative overflow-hidden bg-zinc-50">
          ...
          <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none mb-4">
                <DecryptedText text="ARTICLES" hoverText="DEVELOPERS" />
              </h2>
              <p className="text-xl md:text-2xl font-bold uppercase border-l-4 border-black pl-4">Engineering, architecture, and code.</p>
            </div>
            <Link to="/articles" className="font-black uppercase tracking-widest hover:underline decoration-4 underline-offset-8">
              View The Crypt &rarr;
            </Link>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {recentArticles.map((article, i) => {
              ...
              <CustomTag 
                key={article.slug} 
                {...linkProps as any}
                className="group relative border-8 border-black p-8 bg-white cursor-pointer min-h-[350px] flex flex-col justify-between hover:-translate-y-2 hover:translate-x-2 transition-all shadow-[8px_8px_0px_rgba(0,0,0,1)] hover:shadow-[-8px_8px_0px_rgba(0,0,0,1)]"
              >
                <div>
